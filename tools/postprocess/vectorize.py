...
def get_pred_top2_direction(direction, dim=1):
    ...
    direction = torch.stack([idx1, idx2], dim) - 1 #torch.Size([200, 400, 2])
    return direction


def vectorize(segmentation, embedding, direction, angle_class, morpho_mode='MORPH_CLOSE'):
    ...
    for i in range(1, oh_pred.shape[0]):
        ...
        single_class_inst_mask, single_class_inst_coords = post_processor.postprocess(single_mask, morpho_mode, single_embedding)
